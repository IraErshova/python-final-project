...
async def search_wine_by_meal(meal: str):
    ...
    headers = {"x-api-key": SPOONACULAR_API_KEY}
    r = requests.get(
        "https://api.spoonacular.com/food/wine/pairing", params=payload, headers=headers
    )
...
def format_recipe_response(response) -> str:
    ...
    for recipe in response:
        ...
        used_ingredients = [
            ingredient["original"] for ingredient in recipe["usedIngredients"]
        ]
        ...
        formatted_recipe += "\n".join(
            f"- {ingredient}" for ingredient in missed_ingredients
        )
        formatted_recipe += "\nUsed Ingredients:\n"
        formatted_recipe += "\n".join(
